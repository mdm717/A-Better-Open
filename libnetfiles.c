...
extern int netserverinit(char *hostname, int filemode){
	...
	int serv_ret = 0;
    	...
    	strcpy(CONNECTION.host, buff);													
	CONNECTION.connMode = INVALID_FILE;
    	...
    	if(serv_ret == 0)
	{
        	...
        	CONNECTION.connMode = (CONNECTION_MODE)filemode;
    	}
    	return serv_ret;
}
...
int netopen(const char *pathname, int flags){
    	...
    	if(flags != O_RDONLY && flags != O_WRONLY && flags != O_RDWR)
	{
    	...
    	socket_fd = socketGen(CONNECTION.host);
    	if(socket_fd < 0)
	{
        	errno = 0;
        	h_errno = HOST_NOT_FOUND;
    	...
    	bzero(buff, BUFF_SIZE);
    	sprintf(buff, "%d,%d,%d,%s", NET_OPEN, CONNECTION.connMode, flags, pathname);
